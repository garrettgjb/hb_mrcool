...
class ThermostatAccessory {
    ...
    updateFromDevice(device) {
        ...
        this.heaterCoolerService.updateCharacteristic(Characteristic.CurrentHeaterCoolerState, this.getCurrentState());
        this.heaterCoolerService.updateCharacteristic(Characteristic.TargetHeaterCoolerState, this.getTargetState());
        this.heaterCoolerService.updateCharacteristic(Characteristic.CurrentTemperature, this.getCurrentTemperature());
        this.heaterCoolerService.updateCharacteristic(Characteristic.CoolingThresholdTemperature, this.getTargetTemperature());
        this.heaterCoolerService.updateCharacteristic(Characteristic.HeatingThresholdTemperature, this.getTargetTemperature());
        if (this.supportsFanSpeed) {
        ...
        this.dryModeSwitch?.updateCharacteristic(Characteristic.On, device.latestAction.power === 'on' && device.latestAction.mode === 'dry');
    }
    // -- Active / power ------------------------------------------------------
    getActive() {
        const { Characteristic } = this.platform;
    ...
    setTargetTemperature(value) {
        ...
        const deviceUnitTemp = this.isDeviceFahrenheit()
            ...
            : Math.round(celsius);
        this.platform.log.info('[%s] HomeKit requested target temperature %s°C -> %s%s', this.displayName, celsius, deviceUnitTemp, this.isDeviceFahrenheit() ? 'F' : 'C');
    ...
    setRotationSpeed(value) {
        const step = Math.min(4, Math.max(1, Math.ceil(value / 25)));
